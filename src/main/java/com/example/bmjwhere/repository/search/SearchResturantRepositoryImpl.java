...
import com.example.bmjwhere.entity.QResturant;
import com.example.bmjwhere.entity.QResturantImage;
import com.example.bmjwhere.entity.QReview;
...
@Log4j2
public class SearchResturantRepositoryImpl extends QuerydslRepositorySupport implements SearchResturantRepository {

    public SearchResturantRepositoryImpl() {
        //QuerydslRepositorySupport 클래스의 생성자를 이용하여 도메인 클래스의 객체를 생성
        super(Resturant.class);
    }

    @Override
    public Resturant search1() {
        log.info("search........");

        // QBoard(Q도메인)의 객체를 가져온다.
        QReview review = QReview.review;
        QResturant resturant = QResturant.resturant;

        JPQLQuery<Resturant> jpqlQuery = from(resturant);
        jpqlQuery.leftJoin(review).on(review.resturant.eq(resturant));

        JPQLQuery<Tuple> tuple = jpqlQuery.select(resturant, review.text);
        tuple.groupBy(resturant);
    ...
    public Page<Object[]> searchPage(String type, String keyword, Pageable pageable) {
        ...
        QReview review = QReview.review;
        QResturant resturant = QResturant.resturant;
        QResturantImage resturantImage = QResturantImage.resturantImage;

        // SQL문 작성, SPQLQuery: 쿼리 메소드나 @Query에서 처리하기 어려운 복잡한 쿼리문 작성 시 사용
        JPQLQuery<Resturant> jpqlQuery = from(resturant);
        jpqlQuery.leftJoin(resturantImage).on(resturantImage.resturant.eq(resturant));
        jpqlQuery.leftJoin(review).on(review.resturant.eq(resturant));

        // Tuple: 정해진 엔티티 객체 단위가 아니라 각각의 데이터를 추출하는 경우 사용, 여러 테이블의 집합 함수를 처리하는 것
        JPQLQuery<Tuple> tuple = jpqlQuery.select(resturant, resturantImage, review.grade.avg(), review.count());
        ...
        BooleanExpression expression = resturant.rno.gt(0L);
        // and(): 생성한 조건을 추가
            ...
            for (String t : typeArr) {
                switch (t) {
                    case "t":
                        ...
                        conditionBuilder.or(resturant.title.contains(keyword));
                        break;
                    case "c":
                        // contains(): 함수는 대상 문자열에 특정 문자열이 포함되어 있는지 확인하는 함수
        ...
        sort.stream().forEach(order -> {
            ...
            PathBuilder orderByExpression = new PathBuilder(Resturant.class, "resturant");
