package com.example.bmjwhere.config;

import com.example.bmjwhere.security.handler.ClubLoginSuccessHandler;
import com.example.bmjwhere.security.service.ClubUserDetailsService;
...
//    }
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        //   formLogin();
        //인가, 인증 절차에서 문제가 발생했을 때 로그인 페이지를 보여주도록 지정
        ...
        http.authorizeRequests()
                ...
                .antMatchers("/resturant/list").permitAll()
                .antMatchers("/resturant/cess2").permitAll()
                .antMatchers("/resturant/memberOnly").hasAnyRole("USER", "ADMIN")
                .antMatchers("/resturant/eatDeal2").hasAnyRole("USER", "ADMIN")            
                .antMatchers("/resturant/read").hasAnyRole("USER", "ADMIN")
                .antMatchers("/resturant/adminOnly").hasRole("ADMIN")
                .and()
                .formLogin().loginPage("/resturant/login")
                .loginProcessingUrl("/login_post")
                .defaultSuccessUrl("/resturant/memberOnly")
